    ...
    let framework = poise::Framework::builder()
        ...
        .setup(|ctx, _ready, framework| {
            Box::pin(async move {
                ...
                let latest_my_guilds: Vec<MyGuild> = join_all(latest_guild_futures).await
                    .into_iter()
                ...
                let local_guild_id_set: HashSet<u64> = local_guild_ids.iter().cloned().collect();
                let latest_guild_id_set: HashSet<u64> = latest_my_guilds.iter().map(|my_guild| &my_guild.id).cloned().collect();
                ...
                for MyGuild { id, name: _, members } in &latest_my_guilds {
                    ...
                    let latest_member_ids: Vec<u64> = members.iter().map(|member| member.member_id).collect();
                    ...
                    let local_member_id_set: HashSet<u64> = member_ids_map_by_guild
                        .get(&latest_guild_id)
                        .cloned()
                        .unwrap_or_default()
                        .into_iter()
                    ...
                    let latest_member_id_set: HashSet<u64> = latest_guild_members_set_by_guild
                        .iter()
...
use crate::constants::{EPOCH, JST};
use serenity::model::id::GuildId;
use sqlx::types::chrono::{DateTime, FixedOffset, TimeZone};
...
async fn fetch_my_guild(
    ...
    let epoch_jst = JST.from_local_datetime(&EPOCH.naive_utc()).unwrap();
    let members = partial_guild.members(
        ...
        .filter_map(|member| {
            Some(MyGuildMember {
                ...
                birth: epoch_jst, // 一旦、仮でepochを設定（後にDBの情報で更新）
            })
        })
...
async fn insert_guild_member(
    ...
    sqlx::query!(
        ...
        i64::try_from(member_id)?,
        nickname,
        JST.from_local_datetime(&EPOCH.naive_utc()).unwrap().naive_utc(),
    )
        .execute(pool)
        .await?;
    Ok(())
