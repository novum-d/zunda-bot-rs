...
7DTDの一時外部IPv4は、VM上のゲームプロセスがREADYになった時点でVM自身がDuckDNSへ同期する。Cloud Runは起動完了を待機せず、`/7dtd status`で状態確認とDuckDNS同期の再試行だけを行う。これにより、起動後のDNS更新のためにCloud Runを常時起動したり、レスポンス後のバックグラウンド処理へ依存したりしない。
