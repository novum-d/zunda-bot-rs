...
許可された Discord Guild／Channel／User・Role から `/7dtd status`、管理者から `/7dtd start` と `/7dtd stop` を実行できる。VM内で確認したゲームポートのREADY状態はGuest Attributes経由でBotへ通知する。GCP 構築、Cloud Run 環境変数、セーブ移行、安全停止、backup／復元は Runbook を参照する。
