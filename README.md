...
許可された Discord Guild／Channel／User・Role から `/7dtd status`、管理者から `/7dtd start`、`/7dtd stop`、`/7dtd ip list|add|remove` を実行できる。VM内で確認したゲームポートのREADY状態はGuest Attributes経由でBotへ通知し、IP追加時は管理者向けエフェメラル応答に接続先とゲームサーバーパスワードを表示する。開始・停止はPostgreSQLのトランザクションロックで排他し、別の処理中はVM操作を実行せず使用中メッセージを返す。GCP 構築、Cloud Run 環境変数、セーブ移行、安全停止、接続許可IP、backup／復元は Runbook を参照する。
